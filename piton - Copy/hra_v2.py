...
x_coin = SIRKA
y_coin = random.randrange(50, 281)
...
zvuk_coin = pygame.mixer.Sound("sound_1.wav")
...
while running:
    ...
    text_score = font.render("rychlost " + str(round(CRYCHLOST, 1))  + "        score  = " + str(score) + "      lives = " + str(lives), True, (BILA), (CERNA),)
    ...
    if collision_w_coin:
        ...
        okno.blit(coin, coin_hitbox)
        score = score +1
        CRYCHLOST += true_crychlost
        if CRYCHLOST > crychlost_max or CRYCHLOST == crychlost_max:
            CRYCHLOST = crychlost_max
            if score > 3:
                if true_crychlost < 0.1:
                    pass
                else:
                    true_crychlost *= 0.8
